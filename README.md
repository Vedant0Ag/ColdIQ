...
The project combines **MySQL, SQL analytics, Power BI, Python and Streamlit** into one end-to-end platform.
...
ColdIQ brings these factors together so that different commodities, districts and trade routes can be compared using a common analytical framework.
...
The heavy data processing and scoring are handled in SQL, while Power BI and Streamlit make the results easier to explore and understand.
...
The project currently works with **18 commodities**.
...
The project classifies production into:
...
Production trends are also classified into:
...
The resulting opportunity is classified into different opportunity levels, with a **Primary Reason** explaining the recommendation.
...
Instead of only looking at charts, a user can select locations and commodities and explore the recommendations.
...
The scenario section allows the user to adjust:
...
Some financial calculations are based on modeled assumptions such as:
...
Actual cold-storage operations can differ because of factors such as occupancy, temperature control, maintenance, labour, market fluctuations, handling losses and other operating conditions.

Therefore, financial outputs should be interpreted as **scenario-based estimates useful for comparing commodities**, rather than exact predictions of real-world ROI or payback.

Historical prices and production data also do not guarantee future market behaviour.
...
The Streamlit application can be deployed online so that users can access ColdIQ without running it locally.
